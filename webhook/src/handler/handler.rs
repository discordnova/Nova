...
use common::log::{debug, error, trace};
            ...
            if let (Some(timestamp), Some(signature)) = (timestamp, signature) {
                if let Ok(data) = to_bytes(req.into_body()).await {
                    ...
                    if let Ok(signature_str) = &signature.to_str() {
                        if validate_signature(&self.public_key, &contatenated_data, signature_str) {
                            Ok(data)
                        } else {
        ...
        match self.check_request(req).await {
            Ok(data) => {
                ...
                match utf8 {
                    Ok(data) => {
                        trace!("data: {}", data);
                        match serde_json::from_str::<Interaction>(data) {
                            Ok(interaction) => match interaction.type_ {
                                ...
                                _ => {
                                    ...
                                    let payload =
                                        serde_json::to_string(&common::payloads::CachePayload {
                                            ...
                                            data: Box::new(Dispatch::InteractionCreate(Box::new(
                                                interaction,
                                            ))),
                                        })
                                        .unwrap();
